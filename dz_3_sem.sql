...
SELECT post, SUM(salary) FROM staff AS list
GROUP BY post;  

-- 4. Найдите кол-во сотрудников с специальностью (post) «Рабочий» в возрасте от 24 до 49 лет включительно.
SELECT post AS Должность, COUNT(*) AS количество FROM staff 
WHERE post = 'Рабочий' AND age >= 24 AND age <= 42
GROUP BY post;


-- 5. Найдите количество специальностей
SELECT COUNT(DISTINCT post) AS count_post FROM staff;  -- DISTINCT используется для удаления дубликатов из набора результатов.

-- 6. Выведите специальности, у которых средний возраст сотрудников меньше 30 лет
SELECT post as должность FROM staff GROUP BY post
HAVING AVG(age) < 30
